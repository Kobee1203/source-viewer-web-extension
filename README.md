...
- **Advanced Search**: `Cmd`/`Ctrl`+`F` (or the toolbar button) searches the whole source — not just the rendered viewport. Includes standard text search (live highlighting, match case, whole word, RegExp) and **Structural Search Modes** (**XPath** & **CSS Selectors** for HTML/XML, and **JSONPath** for JSON), complete with a built-in help modal featuring official docs and syntax examples.
